let SPHERE_CLASS = (c) => `class="sphere ${c || ""}"`;
let SYSTEM_CLASS = (c) => `class="system ${c || ""}"`;
let ORBIT_CLASS = (c) => `class="orbit ${c || ""}"`;

function orbitTemplate(object, depth = 1) {
  let heading = `h${depth}`;
  let list = depth % 2 ? "ul" : "ol";
  let style = `style="color: ${object[0].color || "#BDC5C7"}"`;
  return `
...
function solarSystemData() {
  return [
    ...
    [
      ...
      [{ name: "Earth", color: "#05f" }, [[{ name: "Moon", color: "" }]]],
      [
        { name: "Mars", color: "#E67E5A" },
        [[{ name: "Phobos", color: "" }], [{ name: "Deimos", color: "" }]],
      ],
      [
